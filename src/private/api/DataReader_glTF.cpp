...
static bool IsDataURI(Strv uri)
{
  return uri.starts_with("data:") &&
         (uri.find_first_of(";base64,") != Strv::npos);
}
...
{
  if(!isGLTF(src)) throw std::runtime_error("glTF: Bad signature");
...
DataReader::readGLTF(std::istream& src, const ModelOptions& options)
{
  ...
  if(json["buffers"].IsArray())
    out.buffers = readBuffers(
      json["buffers"].AsArray(), m_uriFilter, m_fileReader, options);
  ...
  if(json["images"].IsArray())
    out.images =
      readImages(json["images"].AsArray(), *this, m_uriFilter, options);
