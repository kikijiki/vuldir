...
static D3D12_RESOURCE_DESC makeResourceDesc(const Image::Desc& desc)
{
  ...
  res.Height             = std::max(1u, desc.extent[1]);
...
makeSrvDesc(const ViewRange& range, Format format, Dimension dimension)
{
  ...
  switch(dimension) {
    ...
    case Dimension::e3D:
      ...
      break;
    case Dimension::eCube:
      // Single cubemap is 6 faces; more than 6 means a cube array.
      if(range.layerCount > 6u) {
        srv.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBEARRAY;
        srv.TextureCubeArray.MostDetailedMip = range.mipOffset;
        srv.TextureCubeArray.MipLevels       = range.mipCount;
        srv.TextureCubeArray.First2DArrayFace = range.layerOffset;
        srv.TextureCubeArray.NumCubes =
          std::max(1u, range.layerCount / 6u);
      } else {
  ...
  m_handle{}
{
  ...
  if(m_desc.handle) {
    ...
    if(m_memoryDesc.SizeInBytes == UINT64_MAX)
      throw std::runtime_error("Failed to get image memory requirements");
...
Image::~Image()
{
  ...
  for(auto& view: m_views) {
    // Only SRV/UAV allocate bindless heap slots; RTV/DSV stay CPU-only.
    if(view->type == ViewType::SRV || view->type == ViewType::UAV) {
...
u32 Image::GetSubresourceIndex(u32 mip, u32 layer, u32 plane) const
{
  const u32 layers =
    m_desc.dimension == Dimension::e3D ? 1u : m_desc.extent[2];
  const u32 planes = isStencilFormat(m_desc.format) ? 2u : 1u;
  if(mip >= m_desc.mips)
    throw std::out_of_range("Image subresource mip is out of range");
  if(layer >= layers)
    throw std::out_of_range("Image subresource layer is out of range");
  if(plane >= planes)
    throw std::out_of_range("Image subresource plane is out of range");
  return mip + layer * m_desc.mips + plane * m_desc.mips * layers;
}
